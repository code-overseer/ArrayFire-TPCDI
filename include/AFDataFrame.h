...
public:
    AFDataFrame() = default;
    virtual ~AFDataFrame() { _data.clear(); af::deviceGC(); }
    ...
    inline AFDataFrame equiJoin(AFDataFrame const &rhs, std::string const &lName, std::string const &rName) const { return equiJoin(rhs, _nameToCol.at(lName), rhs._nameToCol.at(rName)); }
